...
| # | Root cause | Fix |
|---|-----------|-----|
| **A** | CM5 Lite **bootloader EEPROM fails to detect the SD card** on cold boot (missing `SD_QUIRKS` / SD-first `BOOT_ORDER`). | Flash an updated EEPROM via `recovery.bin` + a config containing `SD_QUIRKS=1`, `BOOT_ORDER=0xf461`, `SD_BOOT_MAX_RETRIES=5`. **Keep `SD_QUIRKS=1` pinned permanently** — on this hardware it stays load-bearing even on the latest firmware, and a `rpi-eeprom-update -a` that drops it will re-strand the device. See [FIRMWARE-UPDATE](docs/FIRMWARE-UPDATE.md). |
